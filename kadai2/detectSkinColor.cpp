...
struct HSVColor detectSkinColor(struct HSVColor);
	...
	for(int j = 0; j < inImg.getHeight(); j++){
		for(int i = 0; i < inImg.getWidth(); i++){
			...
			if(inImg.getImageMode() == 1){
				...
				hsv = RGB2HSV(trgb);
				hsv = detectSkinColor(hsv);
				trgb = HSV2RGB(hsv);
			}
			outImg.setPnmPixel(i, j, trgb);
		}
	}
	outImg.savePnmImage("skinColor.ppm");
...
struct HSVColor detectSkinColor(struct HSVColor hsv){
	if(hsv.H > 0 && hsv.H < 45 && hsv.S >= 8 && hsv.S <= 50 && hsv.V > 70 && hsv.V <= 95) return hsv;
	else{
		hsv.H = hsv.S = hsv.V = 0;
		return hsv;
	}
