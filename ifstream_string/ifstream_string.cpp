...
void InctImage::loadppmimage(string filename)
{
	...
	if(head_str[0] == 'P')
	{
		...
		magicN = head_str;
		//cout << "Magig Number is " << header.Pnum << endl;
		if(head_str=="P1"){
			cmode=0;
			channel = 1;
		}else if(head_str=="P2"){
...
void Iimc::LoadImageFile(string filename, string nickname){	//名前の重複をさけないとだね
...
void Iimc::CloneImage(string simagename, string dimagename)
{
	...
	if(idx != -1){
		...
		cout << simagename << "という名前の画像はありません" << endl;
		exit(1);
	}
...
int Iimc::getImageIndex(string nickname)
{
	...
	if(itr == gImageNames.end()){
		//printf("%sという名前の画像はありません",nickname);
		//cout << nickname << " という名前の画像はありません" << endl;
		return -1;
	}else{
		return(itr->second); 
	}
...
void  Iimc::CopyImageVal(string source,string nickname)
{
	...
	if(idx1 != -1){
		...
		cout << "Source : " <<  source << " という名前の画像はありません" << endl;
		//printf("Source:%sという名前の画像はありません\n",simagename);
		exit(1);
	}
	if(idx2 != -1){
		dim = gImageList[idx2];
	}else{
		cout << "Dist : " <<  nickname << " という名前の画像はありません" << endl;
		//printf("Dist:%sという名前の画像はありません",dimagename);
		exit(1);
	}
